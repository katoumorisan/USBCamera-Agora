...
public class ExternalVideoInputManager implements IVideoSource {
    ...
    public static final int TYPE_USB_CAMERA = 4;
    ...
    private static ExternalVideoInputManager instance;

    public static ExternalVideoInputManager getInstance(Context context){
        if(instance==null){
            instance = new ExternalVideoInputManager(context);
        }
        return instance;
    }
    ...
    boolean setExternalVideoInput(int type, Intent intent) {
        ...
        switch (type) {
            case TYPE_LOCAL_VIDEO:
                ENGINE.setVideoSource(instance);
